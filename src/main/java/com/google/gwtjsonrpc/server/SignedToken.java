...
public class SignedToken {
  ...
  public String getCookieText(final String cookieName) {
    ...
    } catch (XsrfException | InvalidTokenException e) {
      ok = false;
    }
   ...
   * @throws InvalidTokenException throws when token is null, the empty string, has expired, does
   *     not match the text supplied, or is a forged token.
   */
  public ValidToken checkToken(final String tokenString, final String text, final boolean isUrlSafe)
    ...
    if (tokenString == null || tokenString.length() == 0) {
      throw new InvalidTokenException("Invalid Token! Input token is blank!");
    }
    ...
    try {
      ...
      throw new InvalidTokenException("Invalid Token! Decode BASE64 fails!", e);
    }

    if (in.length != tokenLength) {
      throw new InvalidTokenException("Invalid Token! Token length not match!");
    }
    ...
    if (maxAge > 0 && Math.abs(c - n) > maxAge) {
      throw new InvalidTokenException("Invalid Token! Token is expired!");
    }
    ...
    if (!Arrays.equals(gen, in)) {
      throw new InvalidTokenException("Invalid Token! Token text not match!");
    }
